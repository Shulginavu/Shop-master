...
public class EventsService {
    ...
    private void deliveryOfProduct(int amountProducts, ProductsType productsType) {
        for (int i = 0; i < amountProducts; i++) {
            ProductLocation.warehouse.getAmountProducts().add(productsType);
        }
    }

    private void productTranslation(int amountProducts) {
    ...
    private LinkedList<ProductsType> destroyProducts(LinkedList<ProductsType> place, ShopWorkService shopWorkService) {
    ...
    private LinkedList<ProductsType> buyProducts(int amountPurchases) {
        RandomService random = new RandomService();
    ...
    private void discountSetting(int discountPercent, ProductsType productsType) {
        productsType.setDiscount(discountPercent);
    }

    private ProductsType getRandomProduct() {
        RandomService random = new RandomService();
